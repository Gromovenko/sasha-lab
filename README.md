...
Измеренная пауза не лежит мёртвым отчётом: `harvest/sources.js` перекрывает ею
записанное в реестре число, если замер свежий (≤ 30 дней) и не быстрее секунды.
Числа `delayMs` в реестре остаются осторожным стартовым значением и запасным
вариантом, когда плана нет. Сбор идёт в **одном экземпляре на сервер** — и
очередь, и воскресный крон берут общий `flock /tmp/sashalab-harvest.lock`:
без него недельный крон сел бы поверх идущего сутками полного захода и удвоил
частоту запросов к каждому чужому сайту.
